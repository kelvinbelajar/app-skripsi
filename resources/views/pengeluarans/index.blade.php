<x-app-layout>
    ...
    <div class="p-4 bg-white rounded-lg shadow-xs">
        ...
        <div class="flex justify-end mb-4 gap-2">
            <a href="{{ route('printAllPengeluaran') }}" target="_blank"
                ...
                <table class="w-full whitespace-no-wrap">
                    <thead>
                        <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase bg-gray-50 border-b">
                            ...
                            <th class="px-4 py-3">Nominal Pengeluaran</th>
                            <th class="px-4 py-3">Bukti Pengeluaran</th>
                    ...
                    <tbody class="bg-white divide-y">
                        @forelse ($pengeluarans as $pengeluaran)
                            <tr class="text-gray-700">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    {{ $pengeluaran->acara->nama_acara }}
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    {{ $pengeluaran->tanggal_pengeluaran}}
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    {{ number_format($pengeluaran->nominal_pengeluaran, 0, ',', '.') }}
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    @if ($pengeluaran->bukti_pengeluaran)
                                    <a href="{{ asset('storage/bukti_pengeluaran/' . $pengeluaran->bukti_pengeluaran) }}" target="_blank" class="text-blue-600 hover:underline">View File</a>
                                    @else
                                        N/A
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-sm flex space-x-2">
                                    ...
                                    <a href="{{ route('printPengeluaranbyID', $pengeluaran->id) }}" target="_blank"
                                    ...
                                    <form action="{{ route('pengeluarans.destroy', $pengeluaran->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this pengeluaran?');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-3 text-sm text-gray-700 text-center">No Pengeluaran found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>

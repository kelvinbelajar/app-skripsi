<x-app-layout>
    ...
    <div class="p-4 bg-white rounded-lg shadow-xs">
        ...
        <div class="flex justify-end mb-4 gap-2">
            <a href="{{ route('printAllAcara') }}" target="_blank"
                ...
                <table class="w-full whitespace-no-wrap">
                    <thead>
                        <tr
                            ...
                            <th class="px-4 py-3">Estimasi Pengunjung</th>
                            <th class="px-4 py-3">Biaya Tiket</th>
                            <th class="px-4 py-3">EO</th>
                    ...
                    <tbody class="bg-white divide-y">
                        @forelse ($acara as $item)
                            <tr class="text-gray-700">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    <img src="{{ asset('storage/acaras/' . $item->gambar) }}"
                                        alt="{{ $item->nama_acara }}" class="h-16 w-16 object-cover">
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    {{ $item->estimasi_pengunjung }}
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    {{ $item->biaya_tiket }}
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    {{ $item->event_organizer->nama_organisasi }}
                                    {{-- view as the name of event organizer --}}
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    {{ $item->anggaran }}
                                </td>
                                <td class="px-4 py-3 text-sm flex space-x-2">
                                    <a href="{{ route('acaras.show', $item->id) }}"
                                    ...
                                    <a href="{{ route('printAcarabyID', $item->id) }}" target="_blank"
                                    ...
                                    <a href="{{ route('acaras.edit', $item->id) }}"
                                    ...
                                    <form action="{{ route('acaras.destroy', $item->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"

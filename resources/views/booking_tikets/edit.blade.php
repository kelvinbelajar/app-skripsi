<x-app-layout>
    ...
    <div class="p-4 bg-white rounded-lg shadow-xs">
        ...
        <form action="{{ route('booking_tikets.update', $bookingTiket->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <select id="id_acara" name="id_acara" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50">
                    ...
                    @foreach ($acaras as $acara)
                        <option value="{{ $acara->id }}" {{ $bookingTiket->id_acara == $acara->id ? 'selected' : '' }}>
                            {{ $acara->nama_acara }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <input type="text" id="nama_lengkap" name="nama_lengkap" value="{{ old('nama_lengkap', $bookingTiket->nama_lengkap) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50">
                @error('nama_lengkap')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" id="notelp" name="notelp" value="{{ old('notelp', $bookingTiket->notelp) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50">
                @error('notelp')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="email" id="email" name="email" value="{{ old('email', $bookingTiket->email) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50">
                @error('email')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                @if ($bookingTiket->bukti_bayar)
                    <p class="mt-2 text-sm text-gray-600">Current file: <a href="{{ asset('storage/bukti_bayar/' . $bookingTiket->bukti_bayar) }}" target="_blank" class="text-blue-600 hover:underline">View Current File</a></p>
                @endif
            </div>

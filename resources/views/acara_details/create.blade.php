<x-app-layout>
    <x-slot name="header">
        {{ __('Create Acara Detail') }}
    </x-slot>

    <div class="p-4 bg-white rounded-lg shadow-xs">
        <!-- Form start -->
        <form action="{{ route('acara_details.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-4">
                ...
                <select id="id_eo" name="id_eo"
                    ...
                    @foreach ($eventOrganizer as $event_organizer)
                        <option value="{{ $event_organizer->id }}">{{ $event_organizer->nama_organisasi }}</option>
                    @endforeach
                </select>
                @error('id_eo')
            ...
            <div class="mb-4">
                ...
                <select id="id_acara" name="id_acara"
                    ...
                    @foreach ($acara as $acara)
                        <option value="{{ $acara->id }}">{{ $acara->nama_acara }} | {{ $acara->kategori }}</option>
                    @endforeach
                </select>
                @error('id_acara')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="mb-4">
                ...
                <select id="id_jadwal" name="id_jadwal"
                    ...
                    @foreach ($jadwal as $jadwal)
                        <option value="{{ $jadwal->id }}">{{ $jadwal->tanggal_mulai }} |
            ...
            <div class="mb-4">
                ...
                <select id="id_lokasi" name="id_lokasi"
                    ...
                    @foreach ($lokasi as $lokasi)
                        <option value="{{ $lokasi->id }}">{{ $lokasi->address }}</option>
                    @endforeach
                </select>
                @error('id_lokasi')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>

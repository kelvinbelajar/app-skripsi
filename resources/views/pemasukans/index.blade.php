<x-app-layout>
    ...
    <div class="p-4 bg-white rounded-lg shadow-xs">
        ...
        <div class="flex justify-end mb-4 gap-2">
            <a href="{{ route('printAllPemasukan') }}" target="_blank"
                ...
                Cetak Data Pemasukan
            </a>
            <a href="{{ route('pemasukans.create') }}"
                ...
                <table class="w-full whitespace-no-wrap">
                    <thead>
                        <tr
                            ...
                            <th class="px-4 py-3">Tanggal Pemasukan</th>
                            <th class="px-4 py-3">Nominal Pemasukan</th>
                            <th class="px-4 py-3">Bukti Pemasukan</th>
                    ...
                    <tbody class="bg-white divide-y">
                        @forelse ($pemasukans as $pemasukan)
                            <tr class="text-gray-700">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    {{ $pemasukan->acara->nama_acara }}
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    {{ $pemasukan->tanggal_pemasukan }}
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    {{ number_format($pemasukan->nominal_pemasukan, 0, ',', '.') }}
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    @if ($pemasukan->bukti_pemasukan)
                                        <a href="{{ asset('storage/bukti_pemasukan/' . $pemasukan->bukti_pemasukan) }}"
                                            target="_blank" class="text-blue-600 hover:underline">View File</a>
                                    @else
                                        N/A
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-sm flex space-x-2">
                                    <a href="{{ route('pemasukans.show', $pemasukan->id) }}"
                                    ...
                                    <a href="{{ route('printPemasukanbyID', $pemasukan->id) }}" target="_blank"
                                    ...
                                    <a href="{{ route('pemasukans.edit', $pemasukan->id) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-3 text-sm text-center text-gray-500">No pemasukan
                                    found.</td>
                            </tr>
                        @endforelse
                    </tbody>
